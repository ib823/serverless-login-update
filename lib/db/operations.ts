...
export async function popChallenge(key: string) {
  const val = await redis.get<string>(`challenge:${key}`);
  if (val) await redis.del(`challenge:${key}`);
  return val ?? null;
}
...
// OAuth code + refresh (rotation-friendly)
...
export async function getRefreshToken(tokenId: string) {
  const data = await redis.get<string>(`refresh:${tokenId}`);
